...
class Chromium(Browser):
    ...
    def _find_res(self):
        ...
        if self.os == 'linux':
            res_folder = self._expand("~/.config/chromium/Default/Local Storage/")
            ...
            res_folder = self._expand("%LOCALAPPDATA%\\Chromium\\User Data\\Default\\Local Storage\\")

        elif self.os == "darwin":
            res_folder = self._expand("~/Library/Application Support/Chromium/Default/")
    ...
    def backup(self):
        if self.path:
            fname = "chromium.{}.backup".format(strftime("%Y-%m-%d"))
            try:
